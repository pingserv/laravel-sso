...
class LaravelSSOServer extends SSOServer
{
    ...
    {
        if (!Auth::attempt([config('laravel-sso.authColumn') => $username, 'password' => $password])) {
            return false;
        }
    ...
    {
        try {
            $user = config('laravel-sso.usersModel')::where(config('laravel-sso.authColumn'), $username)->firstOrFail();
        } catch (ModelNotFoundException $e) {
            return null;
        }
